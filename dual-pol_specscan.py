...
if inputs["step"] < 80.0:
    assert inputs["wait"] >= 10.0, "wait has gotta be longer champ"
elif 80.0 < inputs["step"] < 180.0:
    assert inputs["wait"] >= 20.0, "wait has gotta be longer champ"
...
if frnt_connection == False:
    ...
    frnt.move_relative(from_d(5.0))
    time.sleep(5.0)
    frnt.move_absolute(from_d(0.0))
    time.sleep(5.0)
frnt_connection = is_mtr_connected(frnt)
if frnt_connection:
...
if did_break:
    print(
        "an error occured during collection, nothing has been saved, closing connections"
    )
    frnt.close()
    bck.close()
    spectrum.close()
